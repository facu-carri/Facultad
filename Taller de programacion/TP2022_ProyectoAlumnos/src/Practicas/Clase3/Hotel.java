...
/*
Un hotel posee N habitaciones. De cada habitación conoce 
costo por noche, 
si está ocupada 
y, en caso de estarlo, guarda el cliente que la reservó (nombre, DNI y edad).
(i) Genere las clases necesarias. Para cada una provea métodos getters/setters adecuados.
(ii) Implemente los constructores necesarios para iniciar: 

los clientes a partir de nombre,
DNI, 
edad; 
el hotel para N habitaciones, 
cada una desocupada 
y con costo aleatorio entre 2000 y 8000.
...
public class Hotel {
	...
	public Hotel(int dimF) {
		this.dimF = dimF;
		rooms = new Habitacion[dimF];
		for (int i = 0; i < dimF; i++){
			this.rooms[i] = new Habitacion();
		}
	}
	
	public void aumentar(int aumento){
		for (int i = 0; i < dimF; i++){
			rooms[i].setCosto(rooms[i].getCosto() + aumento);
		}
	}
	
	public void addCliente(int id, String nombre, int dni, int edad){
		rooms[id] = new Habitacion(nombre, dni, edad);
	}
	...
	public String toString(){
		String str = "";
		for (int i = 0; i < dimF; i++){
			Habitacion h = rooms[i];
			str += "{Habitación " + i + ": " + h.getCosto() + ", " + h.isOcupada();
			if(h.isOcupada()){
				str += ", " + h.getNombre() + ", " + h.getDNI() + ", " + h.getEdad();
			}
			str += "} \n";
		}
		return str;
